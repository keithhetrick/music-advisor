#!/usr/bin/env python3
"""
backfill_tier2_from_rodolfo.py

Map rodolfo 1.2M Spotify tracks onto Tier 2 (spine_master_tier2_modern_lanes_v1)
by (year, slug) to fill missing audio. Only updates rows where has_audio != 1.
Tier 1/3 untouched.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Backfill Tier 2 audio from rodolfo 1.2M Spotify set.")
    default_db = get_historical_echo_db_path()
    p.add_argument("--db", default=str(default_db), help=f"SQLite DB path (default: {default_db}).")
    p.add_argument("--table", default="spine_master_tier2_modern_lanes_v1", help="Tier 2 table name.")
    p.add_argument("--echo-tier", default="EchoTier_2_YearEnd_Top100_Modern", help="Tier 2 echo label.")
...
def main() -> None:
    ...
    print(f"[INFO] Tier 2 rows needing audio: {len(targets)}")
    ...
    for r in targets:
        ...
        if args.dry_run:
            print(f"[DRY] would update Tier2 id={r['id']} ({year} {r['artist']} — {r['title']})")
            continue
    ...
    if not args.dry_run:
        conn.commit()
    print(f"[INFO] Updated Tier 2 rows: {updated}")
