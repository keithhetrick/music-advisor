...
- macOS queue hardening: default command/workdir/env are absolute and PATH-normalized for the Automator pipeline, the runner no longer force-kills at 120s (optional timeout via `MA_APP_RUN_TIMEOUT`), “Clear (all)” now wipes queue/outbox/cache + badges immediately, queue reset persists to disk, and sidecar/outbox paths live under Application Support. Command runner env mirrors `REPO`/`PYTHONPATH` and venv/bin to match manual pipeline runs. Stop/Clear now also terminates the in-flight automator process instead of only canceling pending items.
...
- macOS host testing polish: slow stop/restart queue tests now run by default (stress/bench remain opt-in), added queue micro-benchmark target, temp-path lint script, Taskfile targets for lint/stress/bench/publish coverage, and a coverage bundler that collects unit/UI coverage plus the latest xcresult when present into `build/coverage-latest/`. Docs updated with commands, Taskfile prereqs, CI guidance (per-PR vs nightly sweeps), a nightly macOS workflow (unit/UI with coverage + stress/bench + artifacts), and a hardened-runtime note for release builds.
