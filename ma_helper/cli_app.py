...
def _maybe_first_run_hint(command: str) -> None:
    """Lightweight first-run nudge; best-effort (ignore write errors)."""
    cfg = load_favorites()
...
def cmd_quickstart() -> int:
    try:
        ...
        hint = lambda msg: None  # type: ignore
        print_ma_banner()
    _maybe_first_run_hint("quickstart")
    tips = [
...
def _post_verify_hint():
    try:
        from ma_helper.ui_world import hint
        hint("Tip: ma dashboard --json | ma ci-plan --base origin/main --matrix")
    except Exception:
        pass


def _post_dashboard_hint():
    try:
        from ma_helper.ui_world import hint
        hint("Tip: ma affected --base origin/main | ma verify | ma ci-plan --matrix")
    except Exception:
        pass
...
def main(argv=None) -> int:
    ...
    _maybe_first_run_hint(args.command)
    projects = orch.load_projects()
