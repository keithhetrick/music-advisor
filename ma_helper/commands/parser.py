...
def build_parser() -> argparse.ArgumentParser:
    ...
    ta_p.add_argument("--cache", nargs="?", choices=["off", "local", "restore-only"], const="local", default="off", help="Skip unchanged projects (hash-based cache; default local when flag is provided without value)")
    ...
    ta_p.add_argument("--live", action="store_true", help="Enable live split-pane rendering (Rich)")
    ta_p.add_argument("--no-live", action="store_true", help="Disable live split-pane rendering (use plain output)")
    ta_p.add_argument("--json", action="store_true", help="Emit machine-readable summary (no tables)")
    ...
    aff_p.add_argument("--cache", nargs="?", choices=["off", "local", "restore-only"], const="local", default="off", help="Skip unchanged projects (hash-based cache; default local when flag is provided without value)")
    ...
    aff_p.add_argument("--live", action="store_true", help="Enable live split-pane rendering (Rich)")
    aff_p.add_argument("--no-live", action="store_true", help="Disable live split-pane rendering (use plain output)")
    aff_p.add_argument("--json", action="store_true", help="Emit machine-readable summary (no tables)")
    ...
    ui = sub.add_parser("ui", help="Full-screen TUI (Textual)")
    ui.add_argument("--interval", type=float, default=1.0, help="Refresh interval seconds (default 1.0)")
    ui.add_argument("--duration", type=int, default=0, help="Seconds to run (0 = until quit)")
    ui.add_argument("--results", help="Override results file path (default: cache last_results.json)")
    ui.add_argument("--logs", help="Override log file path (default: ~/.ma_helper/logs/ma.log)")

    tg = sub.add_parser("tasks-run", help="Run tasks from ma_helper.toml with deps/outputs (Nx/Turbo style)")
    tg.add_argument("name", help="Task name (from ma_helper.toml [tasks.<name>])")
    tg.add_argument("--config", default=None, help="Path to config (default ma_helper.toml)")
    tg.add_argument("--cache", action="store_true", help="Skip task if outputs are fresher than inputs")
    tg.add_argument("--keep-going", action="store_true", help="Continue deps even if a task fails")
    tg.add_argument("--remote-cache", help="Remote cache endpoint (stub; ready for S3/Redis backends)")
    tg.add_argument("--json", action="store_true", help="Emit final summary as JSON")
    tg.add_argument("--html", help="Write final summary as HTML to the given path")
    ...
    git_branch.add_argument("project", nargs="?", default=None, help="Project name to include in branch name")
    ...
    tour = sub.add_parser("tour", help="Guided tour/breadcrumbs (Rich if available)")
    tour.add_argument("--reset", action="store_true", help="Reset tour progress to the first step")
    tour.add_argument("--advance", action="store_true", help="Manually mark the next step complete (optional; auto-advances on real commands)")
    ...
    cache_sub.add_parser("clear", help="Clear cache data")
    cache_explain = cache_sub.add_parser("explain", help="Explain cache entry for a task")
    cache_explain.add_argument("--task", required=True, help="Task name (e.g., audio_engine_test)")
    for cp in cache_sub.choices.values():
        cp.add_argument("--json", action="store_true", help="Emit JSON for stats/explain")

    graph = sub.add_parser("graph", help="Project graph")
    graph.add_argument("--format", choices=["ansi", "mermaid", "dot"], default="ansi", help="Output format")
    graph.add_argument("--filter", help="Substring filter on name/path")
