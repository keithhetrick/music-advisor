...
BN="$(basename "${AUDIO%.*}")"
TS="$(date +%Y/%m/%d)"
OUTROOT="$HOME/music-advisor/features_output/$TS/$BN"
mkdir -p "$OUTROOT"
...
if py_file "tools/ma_audio_features.py"; then
  python tools/ma_audio_features.py \
    ...
    echo "[ma_audio_features] failed → writing minimal features"
    python - "$AUDIO" "$FEATURES_JSON" <<'PY'
import sys, json, soundfile as sf, pyloudnorm as pyln, numpy as np, os
p,out = sys.argv[1], sys.argv[2]
y,sr=sf.read(p); 
if getattr(y,'ndim',1)>1: y=y.mean(axis=1)
lufs=pyln.Meter(sr).integrated_loudness(y)
d={"path":p,"sr":sr,"dur_sec":len(y)/sr,"r128":{"lufs":float(lufs)}}
os.makedirs(os.path.dirname(out), exist_ok=True)
json.dump(d, open(out,"w"), indent=2)
print(f"[fallback features] wrote {out}")
PY
  }
else
  echo "[ma_audio_features] SKIP (tool not found) → writing minimal features"
  python - "$AUDIO" "$FEATURES_JSON" <<'PY'
import sys, json, soundfile as sf, pyloudnorm as pyln, numpy as np, os
p,out = sys.argv[1], sys.argv[2]
y,sr=sf.read(p); 
