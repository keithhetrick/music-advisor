...
CLIs exposed: `music-advisor`, `music-advisor-smoke` (optional if you’re scripting outside GPT).

Local smoke (outside GPT):
```bash
music-advisor-smoke payload.json --market 0.48 --emotional 0.67 --round 3
```
Expect a JSON advisory with `HCI_v1`, `Baseline`, `Goldilocks`, and structural/TTC blocks populated.

Sample payload: `Examples/sample_payload_minimal.json`. Swap in extractor output for real runs.

More docs:
- Quick map: `Docs/README_Map.md`
- Smoke steps: `Docs/README_Smoke_Checklist.md`
- QA ranges: `QA/ExpectedRanges_v1.json`
