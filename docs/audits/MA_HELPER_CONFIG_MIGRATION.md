...
This document analyzes the current global configuration pattern in ma_helper and proposes a migration to immutable configuration objects. The current system has **14 mutable globals** across 2 files that are mutated at runtime via `apply_config()` and the `--root` flag.
