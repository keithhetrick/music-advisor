...
- **Quick commands (shortlist)** — `make quick-check` (or `infra/scripts/quick_check.sh`); `automator.sh <audio>` for the 12-file payload (adds `<stem>.tempo_norms.json` + tempo overlay in `.client.rich.txt`); `python infra/scripts/data_bootstrap.py --manifest infra/scripts/data_manifest.json` for data/public bootstrap.
